...
# login function
from user.models import BookTable
    ...
    if 'restaurant_email' in request.session:
        if request.method == 'POST':
            ...
            if food_item.objects.filter(item_name=item_name,restaurant_id=res_id,item_price=item_price,dish_category=item_dish_category,dish_type=dish_type_item).exists():
                ...
                data = food_item(item_name=item_name,restaurant_id=res_id,item_image=food_image,item_price=item_price,dish_category=item_dish_category,dish_type=dish_type_item,item_description=item_description)
                data.save()
...
def update_food_item(request):
    if 'restaurant_email' in request.session:
        if request.method == "POST":
            ...
            food_data.item_description=request.POST['item_description']
...
def tables_booking(request):
    if 'restaurant_email' in request.session:
        obj=restaurant.objects.get(restaurant_email=request.session['restaurant_email'])
        rest_id=obj.id
        table_bookings=BookTable.objects.filter(rest_id=rest_id).order_by('-id')
        return render(request,"restaurant/tables_booking.html",{"table_bookings":table_bookings})
    else:
        messages.error(request, 'session expired ! Login Again')
        return redirect(login)

def confirm_table_booking(request,id):
    if 'restaurant_email' in request.session:
        obj=BookTable.objects.get(id=id)
        obj.status="confirm"
        obj.save()
        return redirect(tables_booking)
    else:
        messages.error(request, 'session expired ! Login Again')
        return redirect(login)


def cancel_table_booking(request,id):
    if 'restaurant_email' in request.session:
        obj=BookTable.objects.get(id=id)
        obj.status="cancel"
        obj.save()
...
def profile(request):
    if 'restaurant_email' in request.session:
        profile_data=restaurant.objects.get(restaurant_email=request.session['restaurant_email'])
        type=type_of_restaurant.objects.all()

        return render(request, 'restaurant/profile.html',{"profile_data":profile_data,"type":type})
    else:
        messages.error(request, 'session expired ! Login Again')
        return redirect(login)
...
def update_profile(request):
    if 'restaurant_email' in request.session:
        ...
        if 'restaurant_name' in request.POST:
            data=restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_name=request.POST['restaurant_name']
            data.save()
        if 'restaurant_mobile_number' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_mobile_number = request.POST['restaurant_mobile_number']
            data.save()
        if 'restaurant_address' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_address = request.POST['restaurant_address']
            data.save()
        if 'restaurant_city' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_city = request.POST['restaurant_city']
            data.save()
        if 'restaurant_state' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_state = request.POST['restaurant_state']
            data.save()
        if 'restaurant_pincode' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_pincode = request.POST['restaurant_pincode']
            data.save()
        if 'restaurant_type' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data1=type_of_restaurant.objects.get(id=request.POST['restaurant_type'])
            data.restaurant_type = data1
            data.save()
        if 'restaurant_description' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_description = request.POST['restaurant_description']
            data.save()
        if 'restaurant_password' in request.POST:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_password = request.POST['restaurant_password']
            data.save()

        if 'restaurant_logo' in request.FILES:
            data = restaurant.objects.get(restaurant_email=restaurant_email)
            data.restaurant_logo = request.FILES['restaurant_logo']
            data.save()
